...
const initialDiscussions = [
  {
    ...
    name: "Maya Thompson",
    class: "Class 7",
    message: "You: List the most recent or active ",
    avatar: "disc.jpg",
    time: "(12 Minute)",
  },
  {
...
export default function DiscussionBoard() {
  ...
  return (
    <div className="bg-white shadow-md rounded-2xl p-4 w-full lg:max-w-[500px] relative">
      {/* Header */}
      ...
      <div className="max-h-52 overflow-y-auto pr-2 scrollbar-hide">
        {(filterRecent ? [discussions[0]] : filtered).map((d) => (
          <div key={d.id} className="p-2 mb-2 rounded-lg">
            {/* Row: Avatar + Name + Class */}
            <div className="flex  gap-3">
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <p className="text-xs mt-3  bg-blue-100 px-2 py-0.5 rounded-md inline-block">
                    {d.class}
                  </p>

                  <p className="text-xs mt-3  ">{d.time}</p>
                </div>
              </div>
            </div>

            {/* Message below */}
            <p className="text-xs text-gray-600 mt-3   font-[Montserrat] font-normal text-[16px] leading-[100%] tracking-[0] ">
              {d.message}
            </p>
          </div>
        ))}
